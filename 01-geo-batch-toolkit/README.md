...
![Python](https://img.shields.io/badge/python-3.10%20|%203.12-blue) ![Version](https://img.shields.io/badge/version-1.0.0-green)
...
Every batch command runs files in parallel (`--workers N`), **isolates failures** (one corrupt file is logged and reported, and the rest of the batch continues), and writes `run_report.json` + `run_report.csv` (status, output path, time taken, error per file).
...
| Tested behaviour on synthetic data | Benchmarks on multi-GB production datasets (planned: real Telangana datasets) |
